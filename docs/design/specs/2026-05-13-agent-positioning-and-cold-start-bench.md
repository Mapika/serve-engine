...
2. **Sanity-check the numbers.** If the cold-vs-warm gap isn't dramatic (≥5× on First TTFT specifically — see Success criteria), the launch story has to change before any README rewrite. Stop and reassess before continuing.
...
- Stdout: a markdown table that's copy-pasteable into the README:
  ...
  |----------------------|--------------------:|---------------------:|--------:|
  | Engine ready         |               X.Xs |                 Y.Ys |    Z.Z× |
  | First TTFT           |               X.Xs |                 Y.Ys |    Z.Z× |
  | Total wallclock      |               X.Xs |                 Y.Ys |    Z.Z× |
  ```
...
- `bench_snapshots.py` runs end-to-end on the developer's available single-GPU hardware (RTX PRO 6000 Blackwell or H100) and produces both a JSON file and the stdout table. **The "First TTFT" row** must show ≥5× speedup (cold vs. warm). If <5×, design pauses for reassessment; this is not a rubber-stamp deliverable.
...
- **Snapshot numbers underwhelm.** Mitigation: run the benchmark first (critical-path step 1). If the gap is <5× we reassess before any other work.
